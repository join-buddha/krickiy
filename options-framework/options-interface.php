...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case "checkbox":
			...
			$output .= '<label class="explain" for="' . esc_attr( $value['id'] ) . '">' . nl2br( wp_kses( $explain_value, $allowedtags) ) . '</label>';
			break;
		...
		// Editor
		case 'editor':
			$output .= '<div class="explain">' . nl2br( wp_kses( $explain_value, $allowedtags) ) . '</div>'."\n";
			echo $output;
