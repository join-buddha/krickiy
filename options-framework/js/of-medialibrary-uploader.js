...
/*-----------------------------------------------------------------------------------*/
    ...
    $('.image-select-control').each(function () {

      var $control = $(this),
          $buttonRemove = $control.find('.remove-image'),
          $input = $control.find('input.selected-file-id'),
          $imageContainer = $control.find('.image-container .centered'),
          mediaFrame;
      ...
      mediaFrame.on('select', function() {
        ...
        if ( attachments[0] ) {
          ...
          $control.addClass('image-selected');
        }
      });

      $control.find('.image-select-button, .image-change-button').add($imageContainer.parent()).click(function () {
        mediaFrame.open();
        return false;
      });

      $buttonRemove.click(function () {
        $control.removeClass('image-selected');
        $imageContainer.html('');
        $input.val('');
        return false;
      });
