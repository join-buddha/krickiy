...
class PortfolioMedia extends FluxusPage {
    ...
    function to_array() {
        ...
        $thumbnail_url = $this->get_thumbnail( 'thumbnail' );
    ...
    /**
     * Return meta description or if empty return attachment's post_title.
     */
    function get_alt() {
        ...
        if ( ! $alt ) {
            if ( $this->is_image() ) {
                $post = get_post( $this->meta_attachment_id );
    ...
    /**
     * Returns thumbnail data array, or empty array on failure.
     */
    function get_thumbnail( $size = 'fluxus-thumbnail', $fallback_size = false ) {

        $attachment_id = false;

        if ( $this->is_video() ) {
            if ( is_numeric( $this->meta_screenshot_id ) ) {
                $attachment_id = $this->meta_screenshot_id;
            }
        } else {
            $attachment_id = $this->meta_attachment_id;
        }
        ...
        if ( $attachment_id ) {
            ...
            if ( $fallback_size ) {
                ...
                if ( ! is_array ( $meta_data ) || ! isset( $meta_data['sizes'] ) || ! isset( $meta_data['sizes'][$size] ) ) {
                    $image_data = wp_get_attachment_image_src( $attachment_id, $fallback_size );
                }
            }
            ...
            if ( $image_data ) {
                return array(
                    ...
                    'width'     => $image_data[1],
                    'height'    => $image_data[2],
                    'ratio'     => $image_data[2] ? $image_data[1] / $image_data[2] : ''
                );
            }
        }
